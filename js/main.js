...
/* ═══════════════════════════════════════════════════════════════════════
   HOME — fetch data/home.json, render bio + quote + typewriter
   ═══════════════════════════════════════════════════════════════════════ */
function initHome() {
  fetch('data/home.json')
    ...
    .then(function (data) {
      ...
      if (bioEl && data.bio) bioEl.innerHTML = renderBioMarkdown(data.bio);
  ...
  fetch('data/resume.json')
    ...
    .then(function (data) {
      if (expContainer && data.experience) {
        expContainer.innerHTML = data.experience.map(renderExperienceEntry).join('');
      }
      if (eduContainer && data.education) {
        eduContainer.innerHTML = data.education.map(renderEducationEntry).join('');
      }
    })
    .then(function () { initScrollAnimations(); })
    .catch(function () {
      if (expContainer) expContainer.innerHTML = '<p class="loading">Could not load resume data.</p>';
    ...
    '<div class="timeline-entry">' +
      '<div class="entry-header">' +
        '<a class="entry-company" href="' + escHtml(entry.url || '#') + '" ' +
          ...
          escHtml(entry.company) + extLinkIcon() +
        '</a>' +
  ...
  return (
    '<div class="timeline-entry">' +
      '<div class="entry-header">' +
        '<a class="entry-company" href="' + escHtml(entry.url || '#') + '" ' +
          'target="_blank" rel="noopener noreferrer">' +
          escHtml(entry.institution) + extLinkIcon() +
        '</a>' +
        '<span class="entry-dates">' + escHtml(entry.period) + '</span>' +
      '</div>' +
      '<p class="entry-meta">' +
